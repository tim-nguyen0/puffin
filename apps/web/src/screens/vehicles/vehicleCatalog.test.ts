...
describe("vehicle status", () => {
  it("marks the boot vehicle spawned only while telemetry is live", () => {
    expect(statusFor(x500, true)).toBe("spawned");
    expect(statusFor(x500, false)).toBe("available");
  });

  it("keeps installed but unspawned airframes available", () => {
    expect(statusFor(cessna, true)).toBe("available");
  });

  it("marks airframes missing from our px4 ref as not installed", () => {
    expect(statusFor(tiltrotor, true)).toBe("not-installed");
  });
});
